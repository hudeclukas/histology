...
class AE:
    ...
    def make_ae(self):
        ...
        fl = kl.Flatten()(dl5)
        self.middle = kl.Dense(units=self.units,kernel_initializer='glorot_uniform')(fl)
        d = kl.Dense((self.im_size >> 5) * (self.im_size >> 5) * self.f[0], kernel_initializer='glorot_uniform')(self.middle)
        ...
        ul3 = self.up_layer(ul2, self.f[2])
        # uml3 = self.mid_layer(ul3, self.f[1], 2)
        ul4 = self.up_layer(ul3, self.f[1])
        ul5 = self.up_layer(ul4, self.f[0])
        # uml4 = self.mid_layer(ul5, self.f[0], 3)

        self.outputs = kl.Conv2D(self.channels, (1,1), padding="same", activation="sigmoid")(ul5)
